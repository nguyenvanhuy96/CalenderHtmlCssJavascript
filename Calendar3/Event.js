...
export class Event {
    ...
    showIn(calendar) {
        // todo
        console.log("showIn", calendar);
        if (this.date < dateString(calendar.weekStart) ||
        ...
        if ($(`#${this.id}`).length) {
            ...
            eventSlot = $("<div></div>")
                .attr("id", this.id)
                .addClass("event").click(() => this.clickIn(calendar));
        }
        eventSlot.text(this.title).css("backgroundColor", `var(--color-${this.color})`)
        ...
        const duration =
            (new Date(`${this.date}T${this.end}`).getTime() -
                new Date(`${this.date}T${this.end}`).getTime()) / (1000 * 60);
        if (duration < 45) {
            eventSlot.addClass("shortEvent").removeClass("shortEvent");
        } else if (duration < 60) {
